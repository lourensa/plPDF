...
!> Random Variable FUNction Natural Logarithm, level 2 \n
!!    Z = ln(X)
!!
...
!!    method
!!       z    = ln(x)
!!       zden = xden/(1/x) = xden*x
!!       zcum = xcum
!!     
subroutine rvFunLn2(xval,xden,nx,zval,zden,zcum,nz,exitcode) 
...
! routine error code
!   3: nz too small
!   4: x contains negative values or 0
integer,parameter    :: rtnerrcode = 140000000 +  63*1000  ! routine error code for routine rvFunLn2
...
zmin = log(xval(1))    ! mimimum z-value
zmax = log(xval(nx))   ! maximum z-value
zval(1) = zmin
zval(2) = (zmin+zmax)/2.d0
zval(3) = zmax

call rvFunLn3(xval,xden,nx,zval,zden,zcum,3,exitcode)
...
do k=4,nz
   ...
   call rvFunLn3(xval,xden,nx,zval(iz),zden(iz),zcum(iz),1,exitcode)
   if(exitcode.ne.0) return
...
!!   z=ln(x)
!!   
subroutine rvFunLn3(xval,xden,nx,zval,zden,zcum,nz,exitcode) 
...
! routine error code
integer,parameter    :: rtnerrcode = 140000000 +  73*1000  ! routine error code for routine rvFunLn3
